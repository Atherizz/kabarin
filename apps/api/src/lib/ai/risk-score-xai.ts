...
export async function generateXaiNarrative(
  ...
  const breakdownText = [
    `Kepatuhan Obat: ${breakdown.MA.score}/100 — ${breakdown.MA.note}`,
    `Pola Respons: ${breakdown.RP.score}/100 — ${breakdown.RP.note}`,
    `Beban Keluhan: ${breakdown.SB.score}/100 — ${breakdown.SB.note}`,
    `Riwayat Kejadian: ${breakdown.PHE.score}/100 — ${breakdown.PHE.note}`,
    `Multimorbiditas: ${breakdown.M.score}/100 — ${breakdown.M.note}`,
  ].join("\n");
...
function buildFallbackNarrative(input: XaiInput): string {
  ...
  const highDimensions = Object.values(breakdown)
    .filter((v) => v.score >= 50)
    .map((v) => v.note);
