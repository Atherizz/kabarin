<script lang="ts">
  ...
  async function handleSubmit(e: SubmitEvent) {
    ...
    const payload = {
      ...
      family:
        family.filter((f) => f.name && f.phone).length > 0
...
<form onsubmit={handleSubmit} class="flex flex-col gap-10">
  ...
  <section class="flex flex-col gap-4">
    ...
    <input
      bind:value={phone}
      type="tel"
      placeholder="Nomor WhatsApp (opsional jika pasif)"
      class="w-full rounded-3xl bg-light-darker px-5 py-3.5 text-[16px] outline-none focus:ring-2 focus:ring-brand/40"
    />
  ...
  <section class="flex flex-col gap-4">
    ...
    <div class="grid grid-cols-2 gap-3">
      <button
        type="button"
        onclick={() => (monitoringMode = "active")}
        class={`rounded-3xl px-5 py-4 text-left transition-colors ${
          monitoringMode === "active" ? "bg-brand text-white" : "bg-light-darker text-dark"
        }`}
      >
      ...
      <button
        type="button"
        onclick={() => (monitoringMode = "passive")}
        class={`rounded-3xl px-5 py-4 text-left transition-colors ${
          monitoringMode === "passive" ? "bg-brand text-white" : "bg-light-darker text-dark"
        }`}
      >
  ...
  <section class="flex flex-col gap-4">
    ...
    <textarea
      ...
      placeholder="Diagnosa / riwayat penyakit (e.g. Hipertensi, Riwayat Stroke)"
      rows="2"
  ...
  <section class="flex flex-col gap-4">
    ...
    <select
      bind:value={secondaryVolunteerId}
      class="w-full rounded-3xl bg-light-darker px-5 py-3.5 text-[16px] outline-none focus:ring-2 focus:ring-brand/40"
    >
